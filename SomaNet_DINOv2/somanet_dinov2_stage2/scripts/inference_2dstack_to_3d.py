...
Pipeline
--------
Pass 1  Per-slice 2D inference (DINOv2 tiled forward -> affinity seeded
         watershed -> agglomeration -> semantic/morphological post-processing).
         Caches the per-slice instance labels and per-instance mean
         L2-normalised embeddings, so re-stitching does not re-run the GPU.
Stitch  Geometric post-processing chain from the Pass-1 cache
         (utils/postprocess.py): z-overlap re-link (relink_max_gap = 5, "g5"),
         bad-slice bridging, column-solidity hole filling, transient / z-span
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(self.infer2d.test_loader):
                ...
                with torch.amp.autocast(
                    ...
                    # DINOv2: tile the slice into fixed-size windows (positional
                    # embedding is size-locked to the training crop) and blend.
                    embedding, pred_mask_probability = self.infer2d.tiled_forward(
                        inp_img,
                    )
                ...
                # Boundary map from affinity
                boundary_map = self.infer2d._compute_boundary_map(pred_aff_np)
