...
class Trainer:
    ...
    def __init__(self, config: Union[str, Dict[str, Any]]):
        ...
        # Cast all BatchNorm layers to the specified dtype, even if not float32
        #self.cast_batchnorm_to_dtype(self.teacher_model_handler.model, self.dtype)
        #self.cast_batchnorm_to_dtype(self.student_model_handler.model, self.dtype)
    ...
    def force_model_to_dtype(self, model: nn.Module, target_dtype: torch.dtype) -> None:
        """
        No-op: keep model in fp32 and rely on torch.amp.autocast (already wrapping the
        forward calls) to handle mixed-precision. Manually casting the model under
        DataParallel produced cuDNN BN dtype mismatches because replica coalescing
        does not preserve a mixed fp32-BN / bfloat16-rest state.
        """
        print(f"Skipping manual model dtype cast; autocast handles {target_dtype} during forward.")


    # def cast_optimizer_to_dtype(self, optimizer, dtype):
    #     """
    #     Cast all optimizer parameters and state to the specified dtype.

    #     Parameters:
    #         optimizer (torch.optim.Optimizer): The optimizer to cast.
    #         dtype (torch.dtype): The target data type (e.g., torch.bfloat16, torch.float16).
    #     """
    ...
    #     # Cast optimizer state to the specified dtype
    #     for state in optimizer.state.values():
    #         if isinstance(state, dict):  # State could be a dictionary of tensors
    #             for key, value in state.items():
    #                 if torch.is_tensor(value):
    #                     state[key] = value.to(dtype)
    #     print(f"Optimizer successfully cast to {dtype}.")
    ...
    def load_pretrainedModel_teacher_and_student(self):
        ...
        # DINOv2Seg lazily allocates `sp_pos_embed` on first forward; trigger it
        # so the pretrained key loads instead of being re-initialised randomly.
        with torch.no_grad():
            dummy = torch.zeros(
                1, self.config['in_channels'],
                self.config['crop_size'], self.config['crop_size'],
                device=self.device,
            )
            self.teacher_model_handler.model(dummy)
            self.student_model_handler.model(dummy)
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.end_epoch):
            ...
            self.scheduler.step()
            self.update_learning_rate(min_lr_threshold=1e-7)  # below lr_backbone (1e-5) so DINOv2 differential LR isn't clamped up
