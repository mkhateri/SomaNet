...
class Dataset_cls(Dataset):
    ...
    def constraint_checking(self, sample_index, mask_threshold = 0.1, img_threshold = 0.8):
        non_zero_ratio_mask = torch.sum(sample_index['mask'] > 0).item() / sample_index['mask'].numel()
        non_zero_ratio_img = torch.sum(sample_index['img'] > 0).item() / sample_index['img'].numel()
    ...
    def __getitem__(self, index):
        ...
        if self.task_mode == 'SEG':                                         
            sample_index = self.transform(sample_index)
        ...
        if self._is_valid_sample(sample_index) and self.constraint_checking(sample_index, self.mask_nonzero_ratio_threshold):
            ...
            sample_index['labels_affs'], sample_index['affs_mask'] = gen_affs_ours(sample_index['mask'],
                                                                                   offsets=self.offsets,
                                                                                   ignore=False,
                                                                                   padding=True)

            # Generate weight map
            if self.separate_weight_affinity:
            ...
            sample_index['mask_semantic'] = (sample_index['mask'] > 0).long()
