"""Evaluate SomaNet (SwinIR) predictions against the GT mask and save the metrics.

Usage:  python -u run_eval.py 01        (default volume id, for example 01)

Scores somanet_swinir_stage2/inference_results/test_3d_stitched/<vol>/pred_FINAL.nii.gz
...
next to the prediction. No GPU needed.
"""
import os, sys, glob, subprocess
HERE = os.path.dirname(os.path.abspath(__file__))
STAGE2 = os.path.join(HERE, 'somanet_swinir_stage2')
os.chdir(STAGE2)                               
...
pred = os.path.join('inference_results',
                    'test_3d_stitched' + os.environ.get('SW_OUT_TAG', ''),
